...
exports.addUser = newUser => {
  var sql = `insert into relative(elderId, name, email, phone, address, rlUsername, rlPassword, permission) values('${
    newUser.elder_id
  }', '${newUser.name}', '${newUser.email}', '${newUser.phone}', '${
    newUser.address
  }', '${newUser.username}', '${newUser.password}', '${newUser.permission}')`
  return db.insert(sql)
}
exports.logIn = user => {
  var sql = `select * from relative where rlUsername = '${
    user.username
  }' and rlPassword = '${user.password}'`
  return db.load(sql)
}

exports.addElder = elder => {
  var sql = `insert into elder(name, gender, age, ICID, weight, doctorNumPhone) values ('${
    elder.name
  }', '${elder.gender}', ${elder.age}, '${elder.icid}', ${elder.weight}, '${
    elder.doctorPhoneNum
  }')`
  return db.insert(sql)
}
...
exports.addNote = note => {
  var sql = `insert into note(elderId, time, title, script) values('${
    note.elderId
  }', '${note.time}', '${note.title}', '${note.script}')`
  return db.insert(sql)
}
...
exports.updateNote = note => {
  var sql = `update note set time = '${note.time}', title = '${
    note.title
  }', script = '${note.script}' where id = ${note.id}`
  return db.insert(sql)
}
